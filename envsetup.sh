...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^caf_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $CAF_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       CAF_BUILD=
    fi
    export CAF_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the CAF github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var CAF_VERSION)
        ZIPFILE=$CAF_MOD_VERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.caf.device | grep -q "$CAF_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $CAF_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.caf.device | grep -q "$CAF_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $CAF_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/caf/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
